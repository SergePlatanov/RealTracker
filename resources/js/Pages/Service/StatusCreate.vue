...
const form = useForm({
    techno_id: store.state.curTechnoID,
    title: '',
    level: '',
    order: 0,
});
...
const submit = () => {
    form.post(route('status.store'), {
        onFinish: () => form.reset('title', 'level', 'order', 'techno_id'),
    });
};
</script>
...
<template>
    <GuestLayout>
        ...
        <form @submit.prevent="submit">
            <div>
                <InputLabel for="title" value="Статус техпроцесса" />
            ...
            <div>
                <InputLabel for="level" value="Критичность" />
